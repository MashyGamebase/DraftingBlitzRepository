...
public class CardHand : MonoBehaviour
{
    ...
    public void AddCard(Sprite cardIcon, Card card)
    {
        ...
        DeckPool.Instance.AddToDeck(transform.position);

        cards.Add(cardRect);
        ArrangeCards();
    }

    public void RemoveCard(int cardChosen, Sprite cardVisual)
    {
        if (cards.Count == 0) return;

        Destroy(cards[cardChosen].gameObject);

        CardPool.Instance.AddToPool(cardVisual, poolOrigin.position);
